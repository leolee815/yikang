...
class IndexController extends BaseController
{
    ...
    public function main()
    {
        ...
        if($shopset['isfx']) {
        	...
        	//今日销售额
        	$xseToday = number_format($morder->where(array('ispay' => 1))->where($mapToday)->sum('totalprice'), 2);
        	//昨日销售额
        	$xseYesterday = number_format($morder->where(array('ispay' => 1))->where($mapYesterday)->sum('totalprice'), 2);
        	if (!$xseToday) {
        		$xseToday = 0;
        	}
        	if (!$xseYesterday) {
        	...
        	//环比
        	if ($xseYesterday) {
        		$xserate = intval(($xseToday - $xseYesterday) / $xseYesterday * 100);
        	} else {
        		$xserate = $xserate * 100;
        	}
        	$this->assign('xseToday', $xseToday);
        	$this->assign('xseYesterday', $xseYesterday);
        	$this->assign('xserate', $xserate);
        	//支付方式分布
        	//余额支付
        	$pay1 = $morder->where(array('ispay' => 1, 'paytype' => 'money'))->count();
        	//支付宝支付
        	$pay2 = $morder->where(array('ispay' => 1, 'paytype' => 'alipayApp'))->count();
        	//微信支付
        	$pay3 = $morder->where(array('ispay' => 1, 'paytype' => 'wxpay'))->count();
        	$this->assign('pay1', $pay1);
        	$this->assign('pay2', $pay2);
        	$this->assign('pay3', $pay3);
        	$this->assign('paytotal', $pay1 + $pay2 + $pay3);
        }
        //会员关注日志
        ...
        if($shopset['isfx']) {
	        ...
	        $this->assign('zxse', $zxse > 0 ? $zxse : 0);
	        $this->assign('zcje', $zcje > 0 ? $zcje : 0);
	        $this->assign('zxl', $zxl > 0 ? $zxl : 0);
	        $this->assign('zyj', $zyj > 0 ? $zyj : 0);
	        $this->assign('ztx', $ztx > 0 ? $ztx : 0);
        }
        //计算系统运行时间
        $datetime1 = date_create('2016-7-2');
