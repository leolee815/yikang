...
class ShopController extends BaseController
{
    ...
    public function order()
    {
        ...
        $stime = I ( 'stime' ) ? I ( 'stime' ) : '';
        $etime = I ( 'etime' ) ? I ( 'etime' ) : '';
        if($stime && $etime) {
        	$map['paytime'] = array('between', array(strtotime($stime),strtotime($etime)+3600*24-1));
        	$map['ispay'] = 1;
        	$cache = $m->where($map)->page($p, $psize)->order('paytime asc')->select();
    ...
    public function orderExport()
    {
        ...
        //dump($data);
        //die();
        $title = array('订单ID', '订单编号','代金卷ID', '订单总价', '商品总数', '支付价格', '支付类型', '支付时间', '支付宝支付帐号', '邮费', '会员ID', '会员微信ID', '收货姓名', '收货电话', '收货地址', '购买留言', '订单创建时间', '改价时间', '改价原因', '改价操作员', '关闭时间', '关闭原因', '关闭操作员', '退货退款金额', '退货退款申请时间', '退货退款完成时间', '退货快递公司', '退货快递单号', '退货原因', '退货操作员', '订单状态', '发货快递', '发货快递号', '订单商品详情', '分销商ID');
