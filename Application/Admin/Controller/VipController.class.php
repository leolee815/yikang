...
class VipController extends BaseController
{
        ...
        if (count($vipids) <= 1) {
            ...
            foreach ($vipids as $k => $v) {
                ...
                if ($k == 0) {
                    ...
                    $str .= "<div style='float:left;position:absolute'><img style='width:30px' src='" . $temp['headimgurl'] . "'/>" . "&nbsp&nbsp&nbsp&nbsp" . $temp['nickname'] . "（ID：".$temp['id']."）<br>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp↑<br></div>";
                    $str .= "<br><br><br>";
                }
            }
            $str .= "<div style='float:left;position:absolute'><img style='width:30px' src='" . $vip['headimgurl'] . "'/>" . "&nbsp&nbsp&nbsp&nbsp" . $vip['nickname'] . "(当前用户)" . "（ID：".$vip['id']."）". "</div>";
        }
            ...
            if (count($cache) > 0) {
                foreach ($cache as $k => $vip) {
                    if ($vip['type'] == 1) {
                        ...
                        $str .= '<span onclick="javascript:pathopen(this);"><i class="glyphicon glyphicon-plus"></i> ' . $vip['nickname'] . '（ID:'.$vip['id'].'）</span> <a href="javascript:;"></a><span class="numPer redCol">' . $vip['count1'] . '</span><span class="numPer blueCol">' . $vip['count2'] . '</span><span class="numPer greenCol">' . $vip['count3'] . '</span><span class="numPer rouCol">' . $vip['ocount'] . '单：共计' . $vip['osum'] . '</span><span class="numPer eyeCol" data-id="' . $vip['id'] . '" onclick="userInfo(this)"><i class="glyphicon glyphicon-eye-open"></i></span>';
                    } else {
                        $str .= '<li id="node' . $vip['id'] . '" data-id="' . $vip['id'] . '" class="leaf">';
                        $str .= '<span><i class="glyphicon glyphicon-leaf"></i> ' . $vip['nickname'] . '（ID:'.$vip['id'].'）</span> <a href="javascript:;"></a><span class="numPer rouCol" style="color:black">' . $vip['ocount'] . '单：共计' . $vip['osum'] . '</span><span class="numPer eyeCol eyeColCol" data-id="' . $vip['id'] . '" onclick="userInfo(this)"><i class="glyphicon glyphicon-eye-open"></i></span>';
                    }
                    $str .= '</li>';
                }
            }
    ...
    public function vipList()
    {
        ...
        $is_system = I('is_system') ? I('is_system') : '';
        if($is_system) {
        	$is_system == -1 ? 0 : 1;
        	$map['is_system'] = $is_system;
    ...
    public function vipSet()
    {
        ...
        if (IS_POST) {
            ...
            if ($id) {
            	...
            	if($vip['is_system'] && $data['password']) {
                ...
                $re = $m->save($data);
                if (FALSE !== $re) {
                	/** 2016-08-04 edit by mapeijian **/
                	$ms = M('vip_sys');
                	if($ms->where(array('vipid'=>$id))->find()){
                		$ms->where(array('vipid'=>$id))->setField('mobile',$data['mobile']);
                	}
                	/** edit end **/
                    $info['status'] = 1;
                    $info['msg'] = '设置成功！';
    ...
    public function vipExport()
    {
        ...
        if (self::$SHOP['set']['isfx']) {
        	...
        	$title = array('会员ID', '会员层级', '真实电话', '真实姓名', 'E-mail', '金钱', '积分', '经验', 'openid', '微信昵称', '性别', '城市', '省份', '国家', '关注情况', '关注时间', '备注', '创建时间', '交互时间', '是否分销商', '是否正常', '历史佣金', '团队人数', '下线关注次数', '下线取消关注次数', '下线购买次数', '提现金额', '提现姓名', '提现电话', '提现银行', '提现分行', '提现银行所在地', '提现银行卡卡号');
        } else {
        	foreach ($data as $k => $v) {
        		unset($data[$k]['pid']);
        		unset($data[$k]['plv']);
