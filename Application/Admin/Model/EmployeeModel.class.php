...
class EmployeeModel extends Model
{
    ...
    public function caculateAchievement($employees)
    {
        ...
        foreach ($employees as $k => $v) {
            ...
            $map = array();
            $temparr = array();
            $temp = $mvip->field('id')->where(array('employee' => $v['id'],'is_system'=>1))->select();
            foreach ($temp as $vv) {
                array_push($temparr, $vv['id']);
            }

            // 所有预设分销商总数
            $count = $mvip->where(array('id' => array('in', in_parse_str($temparr)),'is_system'=>1))->count();
            $employees[$k]['vip_number'] = $count ? $count : 0;

            // 基本条件
            $map['V.pid'] = array('in', in_parse_str($temparr));
            // 所有订单量
            $count = $morder
            		->alias ( 'O' )
            ...
            // 失败订单量
            $map['O.status'] = array('in', array('4', '7', '6', '0'));// 包括退货中、退货完成、已关闭、已取消
            $count = $morder
		            ->alias ( 'O' )
		            ->join ( 'LEFT JOIN ' . C ( 'DB_PREFIX' ) . 'vip as V on O.vipid=V.id' )
		            ->where($map)
            ...
            // 成功订单量
            $map['O.status'] = array('in', array('2', '3', '5'));// 包括已付款、已发货、已完成
            $count = $morder
            		->alias ( 'O' )
		            ->join ( 'LEFT JOIN ' . C ( 'DB_PREFIX' ) . 'vip as V on O.vipid=V.id' )
		            ->where($map)
            ...
            $payprice = $morder
			            ...
			            ->where($map)
			            ->sum('payprice');
            $employees[$k]['success_order_payprice'] = round($payprice, 2);

            // 当月成交量
            $map['O.ctime'] = array('egt', $beginMonth);
            $count = $morder
            		->alias ( 'O' )
		            ->join ( 'LEFT JOIN ' . C ( 'DB_PREFIX' ) . 'vip as V on O.vipid=V.id' )
		            ->where($map)
            ...
            $payprice = $morder
		            ...
		            ->where($map)
		            ->sum('payprice');
            $employees[$k]['month_order_payprice'] = round($payprice, 2);

            // 当天成交量
            $map['O.ctime'] = array('egt', $beginToday);
            $count = $morder
            		->alias ( 'O' )
		            ->join ( 'LEFT JOIN ' . C ( 'DB_PREFIX' ) . 'vip as V on O.vipid=V.id' )
		            ->where($map)
            ...
            $employees[$k]['today_order_payprice'] = round($payprice, 2);
        }
