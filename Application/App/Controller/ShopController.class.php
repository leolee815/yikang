...
class ShopController extends BaseController
{
    ...
    public function index()
    {
        ...
        $tmpgroup = M('Shop_group')->where(array('status' => 1))->select();
        foreach($tmpgroup as $k => $v) {
        	$indexpic = $this->getPic($v['icon']);
        	$tmpgroup[$k]['indexpic'] = $indexpic['imgurl'];
        }
...
//            }
        ...
        //最新文章
        $artical = M('artical')->limit(3)->order('id desc')->select();
        foreach($artical as $k => $v) {
        	$artical[$k]['listpic'] = $this->getPic($v['listpic']);
        }
    	...
    	foreach ($indexicons as $k => $v) {
    		...
    		if ($indexicons[$k]['soncate']) {
    			...
    			foreach ($son as $kk => $vv) {
    				...
    				$son[$kk]['iconurl'] = $temp['imgurl'];
    				$son[$kk]['ison'] = $type == $vv['id'] ? '1' : '0';
    				$son[$kk]['url'] = U('App/Shop/category', array('type' => $v['id']));
    			}
    			$indexicons[$k]['son'] = 1;
    			$indexicons[$k]['sonlist'] = $son;
    			$indexicons[$k]['url'] = "javascript:;";
    			...
    			$indexicons[$k]['son'] = 0;
    			$indexicons[$k]['url'] = U('App/Shop/category', array('type' => $v['id']));
    		}
    	}
    	$this->assign('indexicons', $indexicons);
    	if(I('type') != null) {
    		$category = M('Shop_cate')->where('id=' . $type)->limit(1)->find();
    		if($category) {
    			$titlename = $category['name'];
    		} else {
    			$titlename = $_SESSION['WAP']['shopset']['name'];
    		}
    	} else {
    		$titlename = '全部商品';
    	}
    ...
    public function orderMake()
    {
        if (IS_POST) {
        	...
        	if($result['status']) {
        		...
        		$data['items'] = $result['result']['allitems'];
        	} else {
    ...
    public function orderOK()
    {
        ...
        if (FALSE !== $rod) {
            ...
            if (self::$WAP['vipset']['cz_exp'] > 0) {
                ...
                //会员合计支付
                $data_vip['total_buy'] = array('exp', 'total_buy+'.$cache['payprice']);
            }
                    ...
                    if ($re) {
                        ...
                        if (FALSE !== $rod) {
                            ...
                            if($email && preg_match($pattern, $email)) {
                            	...
                            	$mail_title = '【'.self::$SHOP['set']['name'].'】新订单，订单号：'.$order['oid'].'，金额：'.$order['payprice'].'元，预设分销商:'.$fxname;
                            	$mail_content = "<html>
									<head>
                            	...
                            	SendMail($email, self::$SHOP['set']['name'], $mail_title , $mail_content);
                            }
                            //前端日志
                            $mlog = M('Shop_order_log');
                            $dlog['oid'] = $order['id'];
