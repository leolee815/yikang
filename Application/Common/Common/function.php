 ...
 * 获取用户顶级预设分销商
 */
function get_topfx($vipid) {
	global $result;
	if($vipid > 0) {
		$m = M('Vip');
		$vip = $m->where('id='.$vipid)->find();
		if($vip['pid']>0) {
			$parent = $m->where('id='.$vip['pid'])->find();
			if(!empty($parent)) {
				$result = $parent['id'];
			}
			get_topfx($parent['id']);
 ...
 * 计算购物车价格
 */
function calculate_price($vipid, $cartids=array()) {
	if(!$vipid) {
	...
	if(empty($cache)) {
		return array('status'=>0,'msg'=>"购物车没有商品");
	}
	if($totalnum<=0) {
