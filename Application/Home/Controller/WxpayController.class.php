...
//		$_SESSION['tmpwxpay']=$wOpt;
    		...
    		if ($order['status'] == 1) {
    			...
    			if (FALSE !== $re) {
    				...
    				$mslog->add($dlog);
    					/*
    					//短信通知商户
    					if(self::$SHOP['bossphone']) {
    						$fxname = get_topfx($order['vipid']);
    						$goodsname = '';
    						...
    						if(count($items)>1) {
    							...
    							$goodsname = substr($goodsname,0,strlen($goodsname)-1);
    						} else {
    							$goodsname = $items[0]['name'];
    						}
    						$sms_data = array(
    								'name' => self::$SHOP['name'],
    								'order' => $order['oid'],
    								'goodsname' => $goodsname,
    								'fxname' => $fxname,
    								'vipaddress' => $order['vipaddress'],
    								'vipname' => $order['vipname'],
    								'vipmobile' => $order['vipmobile']
    						);
    						//$this->sendSms($sms_data, self::$SHOP['bossphone'], 'SMS_16330168');
    					}
    					*/
    				//发送新订单通知邮件给商户
    				$pattern = "/^([0-9A-Za-z\\-_\\.]+)@([0-9a-z]+\\.[a-z]{2,3}(\\.[a-z]{2})?)$/i";
    				$email = self::$SHOP['bossmail'];
    				if($email && preg_match($pattern, $email)) {
    					...
    					$mail_title = '【'.self::$SHOP['name'].'】新订单，订单号：'.$order['oid'].'，金额：'.$order['payprice'].'元，预设分销商:'.$fxname;
    					$mail_content = "<html>
									<head>
									...
									<table>
									<tr>
									<th>订单号: </th><td>".$order['oid']."</td>
									</tr>
									<tr>
									<th>商品名: </th><td>".$goodsname."</td>
									</tr>
									<tr>
									<th>预设分销商: </th><td>".$fxname."</td>
									</tr>
									<tr>
									<th>订单总额: </th><td>".$order['payprice']."元</td>
									</tr>
									<tr>
									<th>收货地址: </th><td>".$order['vipaddress']."</td>
									</tr>
									<tr>
									<th>收货人: </th><td>".$order['vipname']."</td>
									</tr>
									<tr>
									<th>收货电话: </th><td>".$order['vipmobile']."</td>
									</tr>
									<tr>
									<th>支付时间: </th><td>".date('Y-m-d H:i:s',$order['paytime'])."</td>
									</tr>
									<tr>
									<th>下单时间: </th><td>".date('Y-m-d H:i:s',$order['ctime'])."</td>
									</tr>
									</table>
									</body>
									</html>";
    					SendMail($email, self::$SHOP['name'], $mail_title , $mail_content);
    				}
    				if(self::$SHOP['sms']) {
    					//短信通知买家
    					if($order['vipmobile']) {
    						...
    						$this->sendSms($sms_data, $order['vipmobile'], 'SMS_13272270');
    					}
    				}
    				/*
    				//支付成功后设置为分销商
    			...
    			if ($templateid) { // 存在才可以发送模板消息
    				...
    				$paytype = $order['paytype'] == 'wxpay' ? '微信支付' :$order['paytype'];
    				$data['data'] = array(
    ...
    public function cz()
    {
    	...
    	//取ORDER
    	$cache = M('vip_log')->where(array('opid' => $oid))->find();
    	if (!$cache) {
    	...
    	$paysdk->setParameter("notify_url", 'http://' . $_SERVER['HTTP_HOST'] . __ROOT__ . '/Home/Wxpay/cznd/'); //交易通知地址
    	$paysdk->setParameter("trade_type", "JSAPI"); //交易类型
    ...
    //发送短信通知
    public function sendSms($data, $phone, $stpcode) 
    {
    	if(!$data || !$phone || !$stpcode) {
    		return false;
    	}
    	if(!preg_match("/^(13[0-9]|14[0-9]|15[0-9]|17[0-9]|18[0-9])\d{8}$/",$phone)){
    		return false;
    	}
    	$client = new \Util\Alidayu\AlidayuClient();
    	$request = new \Util\Alidayu\Request\SmsNumSend();
    	$smsParams = $data;
    	$smsParams['product'] = '广州逸康';
    	/*
    	$smsParams = [
    			'name'    => $data['name'],
    			'order' => $data['order'],
    	...
    	*/
    	// 设置请求参数
    	$req = $request->setSmsTemplateCode($stpcode)
    	->setRecNum($phone)
    	->setSmsParam(json_encode($smsParams))
    	->setSmsFreeSignName('广州逸康')
    	->setSmsType('normal');
